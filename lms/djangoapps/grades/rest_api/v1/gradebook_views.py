...
class GradebookView(GradeViewMixin, PaginatedAPIView):
    ...
    **Example Request**
        ...
        GET /api/grades/v1/gradebook/{course_id}/?username={username}?limit_history_records={number}
...
class SubsectionGradeView(GradeViewMixin, APIView):
    ...
    **Example GET Response**
    ...
    def get(self, request, subsection_id):
        ...
        limit_history_request_value = request.GET.get('limit_history_records')
        if limit_history_request_value is not None:
            try:
                limit_history_records = int(limit_history_request_value)
            except ValueError:
                limit_history_records = 0

        try:
            override = original_grade.override
            if limit_history_request_value is not None:
                history = override.history.all().order_by('history_date')[:limit_history_records]
            else:
